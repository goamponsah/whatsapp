...
export default function Bookings() {
  ...
  async function load() {
    if (!tenant) return;
    const res = await fetch(`${SERVER}/admin/bookings?tenant_id=${encodeURIComponent(tenant)}&limit=${limit}`, {
  ...
  async function attachRef() {
    const res = await fetch(`${SERVER}/admin/bookings/attach_ref`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', ...(AUTH ? { Authorization: AUTH } : {}) },
      body: JSON.stringify({ booking_id: bookingId, paystack_ref: ref }),
    });
    if (!res.ok) {
  ...
  return (
    <div style={{ padding: 16 }}>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 12 }}>
        ...
        <input type="number" placeholder="Limit" value={limit} onChange={(e) => setLimit(Number(e.target.value) || 50)} />
          ...
          {rows.map((r: any) => (
            <tr key={r.id}>
              ...
              <td style={{ border: '1px solid #ddd', padding: 6 }}>
                {r.user_name || ''}
                <div style={{ fontSize: 12, opacity: 0.8 }}>{r.user_phone}</div>
              </td>
